                  ...
                  <tr key={user.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-3 text-xs font-medium">
                        <button onClick={() => { setError(""); setEditUser(user); }} className="text-slate-300 hover:text-white hover:underline">
                          Edit
                        </button>
                        <button onClick={() => handleDelete(user.id, user.username)} className="text-rose-300 hover:text-rose-200 hover:underline">
                          Delete
                        </button>
                      </div>
                    </td>
      ...
      <Modal title="Create User" open={showCreate} onClose={() => setShowCreate(false)} size="lg">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div>
            ...
            <input id="create-password" name="password" type="password" required minLength={6} className="input-field" autoComplete="new-password" />
          </div>
          <div>
      ...
      <Modal title="Edit User" open={!!editUser} onClose={() => setEditUser(null)} size="lg">
        {editUser && (
          <form onSubmit={handleUpdate} className="space-y-4">
            ...
            <div>
              <label className="label-text" htmlFor="edit-newPassword">New Password <span className="text-xs text-slate-500">(leave blank to keep current)</span></label>
